...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, model, problem, opts, rollout_fn, epoch=0):
        """
        :param rollout_fn: function(model, dataset, opts) -> costs tensor.
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        self.bl_vals = self.rollout_fn(self.model, self.dataset, self.opts).cpu().numpy()
        self.mean = self.bl_vals.mean()
        self.epoch = epoch

    def wrap_dataset(self, dataset):
        print("Evaluating baseline on dataset...")
        return BaselineDataset(dataset, self.rollout_fn(self.model, dataset, self.opts).view(-1, 1))
    ...
    def epoch_callback(self, model, epoch):
        ...
        candidate_vals = self.rollout_fn(model, self.dataset, self.opts).cpu().numpy()
