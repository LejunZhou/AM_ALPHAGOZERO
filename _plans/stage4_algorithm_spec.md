...
**Implementation:** the buffer maintains `_step_index: list[np.ndarray]` of length $N$, with `_step_index[t]` indexing tuple slots currently filled at step $t$. Updated atomically with each push/eviction. O(1) sampling.
